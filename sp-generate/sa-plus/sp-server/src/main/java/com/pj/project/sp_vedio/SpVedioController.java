...
public class SpVedioController {
	...
	public AjaxJson add(SpVedio s){
		...
		s = spVedioService.getById(SP.publicMapper.getPrimarykey());
	...
	public AjaxJson update(SpVedio s){
		int line = spVedioService.update(s);
		return AjaxJson.getByLine(line);
	}
	...
	public AjaxJson updateByNotNull(Long id){
		...
		// 鉴别身份，是否为数据创建者 
		long userId = SP.publicMapper.getColumnByIdToLong(SpVedio.TABLE_NAME, "user_id", id);
		AjaxError.throwBy(userId != StpUserUtil.getLoginIdAsLong(), "此数据您无权限修改");
		// 开始修改 (请只保留需要修改的字段)
		SoMap so = SoMap.getRequestSoMap();
		so.clearNotIn("id", "url", "type", "status", "createBy", "createTime", "updateBy", "updateTime").clearNull().humpToLineCase();	
		int line = SP.publicMapper.updateBySoMapById(SpVedio.TABLE_NAME, so, id);
		return AjaxJson.getByLine(line);
	}
