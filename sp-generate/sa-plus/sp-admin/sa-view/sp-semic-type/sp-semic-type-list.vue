    ...
    <div class="c-panel">
      ...
      <el-table ref="data-table" class="data-table" :data="dataList">
        <sa-td type="selection"></sa-td>
                ...
                <el-table-column label="类型如1：系统2：其他，用户可以自定义" prop="type" >
                    <template slot-scope="s">
                      <el-input v-if="s.row.is_update" v-model="s.row.type" />
                      <span v-else>{{ s.row.type}}</span>
                    </template>
                </el-table-column>
                <el-table-column label="类型说明" prop="name" >
                    <template slot-scope="s">
...
export default {
  ...
  methods: {
    ...
    // 刷新
    f5: function() {
      sa.ajax('/spSemicType/getList', this.p, function(res) {
    ...
    add: function() {
      console.log(123);
      sa.showModel('管理员添加', () => import('./sp-semic-type-add'), { id: -1 });
    },
    // 查看详情
    getInfo: function(data) {
      sa.showModel('管理员详情', () => import('./sp-semic-type-info'), { id: data.id });
    },
    // 查看 - 根据选中的
    ...
    update: function(data) {
      sa.layer.prompt({ title: '修改账号名称' }, function(pass, index) {
        ...
        sa.ajax('/spSemicType/update', { id: data.id, name: pass }, function(res){
          data.name = pass;
          sa.msg('修改成功');
        })
      });
    ...
    updateStatus: function(data) {
      ...
      var ajax = sa.ajax('/spSemicType/updateStatus', { adminId: data.id, status: data.status }, function(res) {
        sa.msg('修改成功');
        is_ok = true;
      });
      // 如果未能修改成功, 则回滚
    ...
    del: function(data) {
      sa.confirm('是否删除，此操作不可撤销', function(){
        sa.ajax('/spSemicType/delete', { id: data.id }, function(res){
    ...
    deleteByIds: function() {
      ...
      sa.confirm('是否批量删除选中数据？此操作不可撤销', function() {
        sa.ajax('/spSemicType/deleteByIds', { ids: ids.join(',') }, function(res) {
