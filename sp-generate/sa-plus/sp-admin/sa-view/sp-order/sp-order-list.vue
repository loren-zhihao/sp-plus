    ...
    <div class="c-panel">
      ...
      <el-table ref="data-table" class="data-table" :data="dataList">
        <sa-td type="selection"></sa-td>
        					...
        					<el-table-column label="支付方式1：支付宝2:微信3:其他" prop="payType" ></el-table-column>
        					<el-table-column label="支付金额" prop="payAmount" ></el-table-column>
        					<el-table-column label="订单来源1:注册2:分享" prop="orderSource" ></el-table-column>
        					<el-table-column label="付款人" prop="drawee" ></el-table-column>
        					<el-table-column label="订单支付时间" prop="orderPayTime" ></el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 刷新
    f5: function() {
      sa.ajax('/spOrder/getList', this.p, function(res) {
    ...
    add: function() {
      console.log(123);
      sa.showModel('管理员添加', () => import('./sp-order-add'), { id: -1 });
    },
    // 查看详情
    getInfo: function(data) {
      sa.showModel('管理员详情', () => import('./sp-order-info'), { id: data.id });
    },
    // 查看 - 根据选中的
    ...
    updateName: function(data) {
      sa.layer.prompt({ title: '修改账号名称' }, function(pass, index) {
        ...
        sa.ajax('/spOrder/update', { id: data.id, name: pass }, function(res){
          data.name = pass;
          sa.msg('修改成功');
        })
      });
    ...
    updateStatus: function(data) {
      ...
      var ajax = sa.ajax('/spOrder/updateStatus', { adminId: data.id, status: data.status }, function(res) {
        sa.msg('修改成功');
        is_ok = true;
      });
      // 如果未能修改成功, 则回滚
    ...
    del: function(data) {
      sa.confirm('是否删除，此操作不可撤销', function(){
        sa.ajax('/spOrder/delete', { id: data.id }, function(res){
    ...
    deleteByIds: function() {
      ...
      sa.confirm('是否批量删除选中数据？此操作不可撤销', function() {
        sa.ajax('/spOrder/deleteByIds', { ids: ids.join(',') }, function(res) {
