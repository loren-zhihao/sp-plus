    ...
    <div class="c-panel">
      ...
      <el-table ref="data-table" class="data-table" :data="dataList">
        <sa-td type="selection"></sa-td>
                ...
                <el-table-column label="关联类型表id" prop="semicDictionaryId" >
                    <template slot-scope="s">
                      <el-input v-if="s.row.is_update" v-model="s.row.semicDictionaryId" />
                      <span v-else>{{ s.row.semicDictionaryId}}</span>
                    </template>
                </el-table-column>
                <el-table-column label="英文简写" prop="abbreviation" >
                    <template slot-scope="s">
                      <el-input v-if="s.row.is_update" v-model="s.row.abbreviation" />
                      <span v-else>{{ s.row.abbreviation}}</span>
                    </template>
                </el-table-column>
                <el-table-column label="描述" prop="description" >
                    <template slot-scope="s">
                      <el-input v-if="s.row.is_update" v-model="s.row.description" />
                      <span v-else>{{ s.row.description}}</span>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="更新人" prop="updateTime" >
                    <template slot-scope="s">
                ...
                <el-table-column label="英文全称" prop="fullName" >
                    <template slot-scope="s">
                      <el-input v-if="s.row.is_update" v-model="s.row.fullName" />
                      <span v-else>{{ s.row.fullName}}</span>
                    </template>
                </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 刷新
    f5: function() {
      sa.ajax('/spSemicChildDictionary/getList', this.p, function(res) {
    ...
    add: function() {
      console.log(123);
      sa.showModel('管理员添加', () => import('./sp-semic-child-dictionary-add'), { id: -1 });
    },
    // 查看详情
    getInfo: function(data) {
      sa.showModel('管理员详情', () => import('./sp-semic-child-dictionary-info'), { id: data.id });
    },
    // 查看 - 根据选中的
    ...
    update: function(data) {
      sa.layer.prompt({ title: '修改账号名称' }, function(pass, index) {
        ...
        sa.ajax('/spSemicChildDictionary/update', { id: data.id, name: pass }, function(res){
          data.name = pass;
          sa.msg('修改成功');
        })
      });
    ...
    updateStatus: function(data) {
      ...
      var ajax = sa.ajax('/spSemicChildDictionary/updateStatus', { adminId: data.id, status: data.status }, function(res) {
        sa.msg('修改成功');
        is_ok = true;
      });
      // 如果未能修改成功, 则回滚
    ...
    del: function(data) {
      sa.confirm('是否删除，此操作不可撤销', function(){
        sa.ajax('/spSemicChildDictionary/delete', { id: data.id }, function(res){
    ...
    deleteByIds: function() {
      ...
      sa.confirm('是否批量删除选中数据？此操作不可撤销', function() {
        sa.ajax('/spSemicChildDictionary/deleteByIds', { ids: ids.join(',') }, function(res) {
